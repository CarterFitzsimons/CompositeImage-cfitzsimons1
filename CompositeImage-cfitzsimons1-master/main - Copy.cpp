...
for(int tx = 0; tx < merged.size() - 1; tx++)  //important note: the tx and ty here are entirely seperate from the tx and ty in the getUserImages() function
	{
		for(int ty = 0; ty < merged[tx].size() - 1; ty++){ //nested for loops go through each pixel and change its value to 0
			...
			rgb.red += (gbr.red/numPhotos);     //changes color values to 0
