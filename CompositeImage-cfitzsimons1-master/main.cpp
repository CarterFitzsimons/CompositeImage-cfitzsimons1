...
/*So the program doesn't currently work, but all of the funcitons are bilt. I was able to successfully turn each pixels rgb values to 0
but whenever I tried to modify and add the rgb values from allImages[i] to the composite image (Bitmap final and vector<vector<Pixel> > merged)
I would get SEGFAULT or SIGTRAP. I tried to look thorough the assembly code to find the breakpoint but couldn't. the other main files were for testing
*/
using namespace std;
...
//Should I be using int * getUserImages() for a pointer instead of a vector <Bitmap> return type?
vector <Bitmap> getUserImages(){
	...
	while (isFinished == false){   //primary loop of the function, checks for isFinished value that will be changed to false when the user inputs DONE
		
		cout << "Please enter a valid .bmp file. If you are finsihed, input DONE" << endl;
		cin >> userInput; // user attempt to enter the file name of the current image
...
Bitmap setToZero(Bitmap final){ //sets the pixels rgb values to 0

//vector< vector<Pixel> > merged
...
Bitmap addAnImage(vector< vector<Pixel> >merged, vector<vector<Pixel> > current){ // divides the R, G, and B values of each pixel in an image by the # of images and adds it into merged bmp

for(int tx = 0; tx < x  ; tx++)  //important note: the tx and ty here are entirely seperate from the tx and ty in the getUserImages() function
	{
		for(int ty = 0; ty < y ; ty++){ //nested for loops go through each pixel and change its value to 0
...
int main(){
	...
	merged = final.toPixelMatrix();
	setToZero(final); //sets a vector of vector of pixels (named merged)to have all its RGB values equal to 0 so we have a black canvas to work with
	...
	for(int i = 0; i <= numPhotos - 1 ; i++){
		...
		addAnImage(merged, current);
		
		
		//if (i == numPhotos-2){                                            //TEST (No longer works until setToZero is fixed)
	...
	final.save("composite-cfitzsimons1.bmp");
	return 0;
}
